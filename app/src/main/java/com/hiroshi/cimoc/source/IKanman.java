...
public class IKanman extends MangaParser {
    ...
    public Request getSearchRequest(String keyword, int page) {
        String url = "";
        if (page==1)
        url = StringUtils.format("https://www.manhuagui.com/s/%s_p%d.html", keyword, page);
    ...
    public SearchIterator getSearchIterator(String html, int page, final String keyword) {
        ...
        return new NodeIterator(body.list("li.cf")) {
            ...
            protected Comic parse(Node node) {
                ...
                String update = node.text(".book-detail > dl > dd:eq(1)");
                update = StringUtils.match("\\d{4}-\\d{2}-\\d{2}",update,0);
                String author = node.text(".book-detail > dl > dd:eq(3) a");
