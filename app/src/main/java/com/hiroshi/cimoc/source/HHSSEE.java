...
public class HHSSEE extends MangaParser {
    ...
    public Request getSearchRequest(String keyword, int page) {
        if (page == 1) {
            String url = "http://www.1manhua.net/comic/?act=search&st=".concat(keyword);
    ...
    public Request getInfoRequest(String cid) {
        String url = StringUtils.format("http://www.1manhua.net/manhua%s.html", cid);
        return new Request.Builder().url(url).build();
    }
    ...
    public List<Chapter> parseChapter(String html) {
        ...
        for (Node node : body.list("#permalink > div.cVolList > ul.cVolUl > li > a")) {
            String title = node.text();
            title = title.replaceFirst(name, "").replaceAll("^\\S*?\\s","").trim();
    ...
    public Request getImagesRequest(String cid, String path) {
        ...
        String url = StringUtils.format("http://www.1manhua.net/page%s/1.html?s=%s", array[0], array[1]);
        return new Request.Builder().url(url).build();
    }
    ...
    public List<ImageUrl> parseImages(String html) {
        ...
        for (int i = 1; i <= page; ++i) {
            list.add(new ImageUrl(i, StringUtils.format("http://www.1manhua.net/page%s/%d.html?s=%s", path, i, server), true));
        }
        return list;
    }
    ...
    private static class Category extends MangaCategory {
        ...
        public String getFormat(String... args) {
            if (!"".equals(args[CATEGORY_SUBJECT])) {
                return StringUtils.format("http://www.1manhua.net/comic/class_%s/%%d.html", args[CATEGORY_SUBJECT]);
            } else if (!"".equals(args[CATEGORY_AREA])) {
                return StringUtils.format("http://www.1manhua.net/comic/class_%s/%%d.html", args[CATEGORY_AREA]);
            } else {
                return "http://www.1manhua.net/comic/%d.html";
            }
        }
    ...
    public Headers getHeader() {
        return Headers.of("Referer", "http://www.1manhua.net");
    }
